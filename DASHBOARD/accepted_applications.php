...
// Fetch accepted adoption applications from the database
$sql = "SELECT application_id, child_id, parent_id, application_date, application_status, review_notes, final_decision_date 
        FROM adoption_applications 
        WHERE application_status = 'accepted'";  // Only fetch accepted applications
...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Accepted Adoption Applications</h1>
        <!--<a href="download_accepted_applications.php" class="btn btn-success">Download Report</a>-->
    </div>

    <div class="container mt-5">
        <h2 class="mb-4">Accepted Adoption Applications</h2>
        <table id="acceptedApplications" class="table table-striped table-bordered">
            <thead>
                <tr>
                    <th>S.No</th>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td><a href='view_accepted_application.php?id={$row['application_id']}' class='btn btn-primary btn-sm'>View</a></td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='9'>No accepted adoption applications found</td></tr>";
                }
                ?>
            </tbody>
        </table>
...
<script>
    $(document).ready(function() {
        $('#acceptedApplications').DataTable();
    });
</script>
